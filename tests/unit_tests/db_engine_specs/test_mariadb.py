...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "mariadb+mysqlconnector://user:password@host/db1?allow_local_infile=1",
            True,
        ),
        ("mariadb://user:password@host/db1?local_infile=0", True),
        (
            "mariadb+mysqlconnector://user:password@host/db1?allow_local_infile=0",
            True,
        ),
...
def test_validate_database_uri_local_infile(sqlalchemy_uri: str, error: bool) -> None:
    """CVE-2024-34693: MariaDB must reject URIs containing local_infile params."""
...
def test_adjust_engine_params_enforces_local_infile(
    sqlalchemy_uri: str, connect_args: dict[str, Any], returns: dict[str, Any]
) -> None:
    """CVE-2024-34693: MariaDB must force local_infile=0 in connect_args."""
    from superset.db_engine_specs.mariadb import MariaDBEngineSpec

    url = make_url(sqlalchemy_uri)
    _, returned_connect_args = MariaDBEngineSpec.adjust_engine_params(url, connect_args)
