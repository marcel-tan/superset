...
def test_import_chart_overwrite_does_not_add_owner(
    mocker: MockerFixture,
    session_with_data: Session,
) -> None:
    """
    Test that overwriting an existing chart does not add the importing user
    as an owner (fixes privilege escalation via re-import).
    """
...
def test_import_new_chart_adds_owner(
    ...
    """
    Test that importing a new chart adds the importing user as an owner.
    """
    mocker.patch.object(security_manager, "can_access", return_value=True)
